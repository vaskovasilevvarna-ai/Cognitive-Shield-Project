# Sprint 2 CMO Structural Contract Boundary Review

Status: Boundary review — before admitting minimal Canonical Message Object (CMO) structural contract behavior.

## Purpose

This document records the Sprint 2 boundary review for the next micro-slice after the controlled CMO minimal shell pass.

The purpose is to decide whether the project may admit a minimal Canonical Message Object (CMO) structural contract while preventing drift into full CMO construction, CMO schema population, CMO field mapping, CMO normalization, CMO enrichment, Agent Communication Protocol (ACP) routing, Analysis generation, runtime pipeline execution, downstream pipeline logic, taxonomy behavior, risk scoring, governance behavior, or output rendering.
...
Can the project admit a minimal CMO structural contract as the next Sprint 2 micro-slice?
...
The project may admit a future minimal CMO structural contract implementation only if it remains:
...
This boundary review does not yet implement CMO structural contract behavior.

## Critical Boundary Distinction

A CMO minimal shell is not a CanonicalMessageObject.

A CMO structural contract is not full CMO construction.

A CMO structural contract is not CMO field mapping.

A CMO structural contract is not CMO schema population.

A CMO structural contract is not ACP handoff.

A CMO structural contract is not an Analysis input bundle.

A CMO structural contract is only a bounded CMO-side structural agreement about the minimal shape required before future CMO construction may be considered.
...
- reading an existing minimal CMO shell;
- checking whether the shell status is `cmo_shell_created`;
- preserving the original DecompositionResult;
- returning a minimal structural contract status;
- returning source and target stage identity;
- exposing a bounded contract object;
- testing that no full CMO, CMO schema, CMO fields, ACP, Analysis, taxonomy, risk, governance, output, or runtime fields are exposed.

Allowed structural contract statuses may include:
...
- CMO normalization;
- CMO enrichment;
- MDS-to-CMO conversion beyond already admitted boundary eligibility and shell status;
- ACP routing;
- ACPBundle creation;
- ACPMessage dispatch;
- Analysis generation;
...
1. The behavior must remain a minimal CMO structural contract only.
2. The behavior must require an existing minimal CMO shell.
3. The behavior must require `cmo_shell_created` status.
...
6. The behavior must not populate a CMO schema.
7. The behavior must not map MDS fields into final CMO fields.
8. The behavior must not normalize MDS content.
9. The behavior must not enrich MDS content.
10. The behavior must not call ACP modules.
11. The behavior must not call Analysis modules.
12. The behavior must not call taxonomy, risk, governance, or output modules.
13. The behavior must not infer hidden or implicit meaning.
14. The behavior must not assess truth.
15. The behavior must not assess evidence.
16. The behavior must not create relation objects.
17. The behavior must not create risk signals.
18. The behavior must not trigger runtime pipeline execution.
19. The original CMO shell object must be preserved unchanged.
20. Tests must prove that forbidden downstream fields are absent.
21. Closure documentation must state that full CMO construction remains on HOLD.
...
- `create_cmo_structural_contract_minimal`
- `build_minimal_cmo_contract`
...
A separate `structural_contract.py` file keeps the contract boundary explicit and reversible.

## Admitted Output Shape

A future CMO structural contract helper may return a minimal dictionary containing only fields such as:
...
The minimal structural contract must not expose:
...
- a valid minimal CMO shell creates a structural contract;
- a non-eligible shell does not create a ready structural contract;
...
This boundary review is justified because the CMO structural contract is the first step after the minimal CMO shell and begins to define the shape of future CMO construction.

After this review, small repository checks may remain in chat unless they reveal a blocker.

A closure note is required only if CMO structural contract behavior is implemented.
...
- CMO structural contract behavior is admitted only as a future review-approved contract boundary;
...
If this review is accepted, create the CMO structural contract pass in compressed mode:
...
- `docs/sprint_2/SPRINT_2_CMO_STRUCTURAL_CONTRACT_PASS_CLOSURE_NOTE.md`

## Verdict Summary

CMO structural contract boundary: ADMITTED FOR CONTROLLED IMPLEMENTATION WITH HARD SAFEGUARDS.
